...
def test_hard_computer_chooses_corner_as_first_move(hard_computer, xo_board):
    board = Board()
    move = hard_computer.move(board)
    assert move in xo_board.get_corners()
...
def test_hard_computer_wins_if_able(hard_computer, xo_board):
    for args in xo_board.get_winning_patterns():
        args = list(args)
        random.shuffle(args)
        a, b, c = args
        board = Board()
...
def test_hard_computer_blocks_if_cant_win(hard_computer, xo_board):
    for args in xo_board.get_winning_patterns():
        args = list(args)
        random.shuffle(args)
        a, b, c = args
...
def test_hard_computer_response_strategy_picks_mid_spot_turn_three(seed, xo_board):
    ...
    move = hard_computer.move(board)
    assert move in xo_board.get_middles()
